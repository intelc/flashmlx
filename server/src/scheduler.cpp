...
void BatchScheduler::prefill_request(Request& req) {
    ...
    // Write updated caches back to the pool without trimming. Decode now uses
    // slice_update into the pre-allocated buffers rather than growing caches
    // with concatenate every token.
    for (int l = 0; l < num_layers; ++l) {
