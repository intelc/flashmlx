...
def bench():
    ...
    # Warmup
    for _ in generate(model, prompt, max_tokens=WARMUP_TOKENS, temperature=0.0, eval_batch_size=32, prealloc_cache=True):
        pass

    tok_s_runs = []
    for _ in range(NUM_RUNS):
        gen_iter = generate(model, prompt, max_tokens=GEN_TOKENS, temperature=0.0, eval_batch_size=32, prealloc_cache=True)
